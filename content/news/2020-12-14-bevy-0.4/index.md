...
A little over a month after releasing Bevy 0.3, and thanks to **X** contributors, **X** pull requests, and our [**generous sponsors**](https://github.com/sponsors/cart), I'm happy to announce the **Bevy 0.4** release on [crates.io](https://crates.io/crates/bevy)!
...
Most of Bevy's high level render abstractions were designed to be incrementally updated, but when I was first building the editor, ECS change detection wasn't implemented. Now that we have all of these nice optimization tools, it makes sense to use them!
...
Rust has a pretty big "reflection" gap. For those who aren't aware, "reflection" is a class of language feature that enables you to interact with language constructs at runtime. They add of form of "dynamism" to what are traditionally static language concepts. 
...
<div class="release-feature-authors">author: @karoffel</div>

@karoffel added a fun example that represents each Bevy contributor as a "Bevy Bird". It scrapes the latest contributor list from git.
